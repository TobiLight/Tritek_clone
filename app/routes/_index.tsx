...
export default function Index() {
	...
	return (
		<main style={{ fontFamily: "system-ui, sans-serif", lineHeight: "1.8" }}>
			...
			<section className="hero h-auto md:h-[450px] lg:h-[600px] lg:max-h-[600px] bg-gray-300 relative">
				<div className={`slide-container w-full relative m-auto h-[inherit] ${isLoaded ? 'visible' : 'hidden'}`}>
					{slideImages.map((img, idx) => {
						return (
							...
							>
								<img src={img} alt="" className="h-[inherit] lg:h-[600px]" />
							</div>
						);
					})}
			...
			<section className="grid lg:grid-cols-2 gap-y-6 lg:flex lg:flex-row lg:gap-x-2">
				<div className="grid gap-y-6 pl-4">
					...
					<div className="lg:flex lg:flex-row mb-5">
						<div className="flex flex-col lg:flex-row gap-y-4 lg:gap-x-4 pl-3 h-[fit-content]">
							<Link to="#" className="bg-[#263786] text-white p-2 w-[fit-content] rounded">Get Started</Link>
						</div>
					</div>
				</div>
					...
					<div className="form">
						...
						<form action="" className="grid md:px-8 lg:w-3/4 mx-auto gap-2">
							<div className="grid gap-4">
								<label htmlFor="">
									Name
									<input type="text" placeholder="John Doe" className="w-full border rounded-md px-4 py-2" />
								</label>
								<label htmlFor="">
									Subject
									<input type="text" placeholder="Project Execution" className="w-full border rounded-md px-4 py-2" />
								</label>
								<label htmlFor="">
									Message
									<textarea name="" id="" cols={30} rows={4} className="w-full border rounded-md px-4 py-2"></textarea>
								</label>
							</div>
							<button className="text-white bg-blue-500 rounded py-2 px-4 flex justify-center items-center gap-1">
